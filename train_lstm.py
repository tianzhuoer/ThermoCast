"""
LSTM 基线训练入口：在 CTD 数据集上从零训练双层 LSTM 温跃层预测模型。

输入与 Reprogram-TSF 对齐：每步使用原始剖面拼接特征
  [depth(1) | T_profile(N) | S_profile(N) | season_norm | doy_sin | doy_cos]
共 1+N+N+3 = 34 维（N=SAMPLE_WINDOW_SIZE=15），训练时加高斯噪声增强。

启动命令
--------
单 GPU（指定数据量）:
  python train_lstm.py --samples 7200 --gpu 1
多 GPU:
  accelerate launch --num_processes=<N> train_lstm.py --samples 7200
...
import configs.lstm_tsf as cfg
from utils import apply_gpu_arg, apply_samples_arg
from models.seq_baselines import load_lstm
...
def _make_raw_feature(ex, is_train: bool) -> torch.Tensor:
    """
    每步拼接原始剖面特征，与 Reprogram 输入对齐：
      [depth_norm(1) | T_prof_zscore(N) | S_prof_zscore(N) | season_norm | doy_sin | doy_cos]
    T/S 做 per-step z-score（与 reprogram_backbone._normalize_inputs 一致），输出 (K, 1+N+N+3)。
    """
    ...
    # per-step z-score 沿空间维度（N），与 reprogram_backbone._normalize_inputs 对齐
    T_arr = np.array(input_feats, dtype=np.float32)          # (K, N)
    T_mean = T_arr.mean(axis=1, keepdims=True)
    T_std  = T_arr.std(axis=1, keepdims=True)
    T_norm = (T_arr - T_mean) / (T_std + 1e-6)

    if has_sal:
        S_arr  = np.array(input_sal, dtype=np.float32)       # (K, N)
        S_mean = S_arr.mean(axis=1, keepdims=True)
        S_std  = S_arr.std(axis=1, keepdims=True)
        S_norm = (S_arr - S_mean) / (S_std + 1e-6)
    else:
        S_norm = np.zeros((K, N), dtype=np.float32)

    depth_col   = np.array(depth_norm, dtype=np.float32).reshape(K, 1)
    context_col = np.array([[season_norm, doy_sin, doy_cos]] * K, dtype=np.float32)

    feat = np.concatenate([depth_col, T_norm, S_norm, context_col], axis=1)  # (K, 34)
